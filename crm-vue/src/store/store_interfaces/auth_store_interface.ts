...
export interface AuthActionsTypes {
  [AuthATypes.LOGIN]({ commit }: AugmentedActionContextAuth,
    payload: {
      email: string, password: string
    }): void;
  [AuthATypes.REGISTER]({ commit }: AugmentedActionContextAuth,
    payload: {
      name: string, email: string, password: string, password_confirmation: string, phone?: string,
    }): void;
